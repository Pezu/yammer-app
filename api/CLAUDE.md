...
- **On-prem bridge / fiscal receipts** (ported from old yammer; app in `bridge/mobile`):
  ...
  (VAT from the product; the tip as an `exempt:true` line → the register's "scutit de TVA" group; its DB steps run in REQUIRES_NEW transactions
...
- `/integrations` — "Peripherals" per location: cash registers and printers, reached over
  TCP (`ip`) or `connection=MOBILE` = attached to a bridge phone: `device_id` (the id the
  app announces in HELLO, picked from `GET /bridge/devices`) + `device_name` (remembered
  for display while the phone is offline, V34). The phone is pure routing — the backend
  forwards the job to it and it drives the USB device. Response carries `online` for
  MOBILE rows. CRUD writes ADMIN/SUPER, list for any authenticated user (`?type=`).
...
- `V32__Integration_mobile.sql`: (superseded by V34) MOBILE rows + `bridge_id`; connection USB → MOBILE.
- `V34__Integration_device_direct.sql`: `integration.device_name`; MOBILE rows folded into their
  devices' `device_id`, `bridge_id` dropped.
