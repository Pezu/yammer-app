...
import { PaymentType, PaymentTypeService } from './payment-type.service';
import { ConfirmDialog } from '../../../../shared/confirm-dialog/confirm-dialog';

@Component({
  selector: 'app-payment-types-page',
  imports: [ReactiveFormsModule, ConfirmDialog],
  templateUrl: './payment-types-page.html',
  styleUrl: './payment-types-page.scss',
})
export class PaymentTypesPage {
  private readonly typeService = inject(PaymentTypeService);
  ...
  readonly pendingDelete = signal<PaymentType | null>(null);
  ...
  load(): void {
    ...
    this.typeService.list().subscribe({
      ...
      error: () => {
        this.error.set('Failed to load payment types.');
        this.loading.set(false);
      },
    });
  }
  ...
  startEdit(type: PaymentType): void {
    this.draft.set(false);
  ...
  private sorted(list: PaymentType[]): PaymentType[] {
    return [...list].sort((a, b) => a.type.localeCompare(b.type));
  }

  private message(err: HttpErrorResponse, action: string): string {
    if (err.status === 409) {
      return 'A payment type with that name already exists.';
    }
    return `Failed to ${action} payment type.`;
  }
}
